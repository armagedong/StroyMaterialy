...
{
    public static async Task ImportAllAsync(IProgress<string>? progress = null)
    {
        var folder = AppConfig.ImportPath;
    ...
    private static async Task ImportUsersAsync(string path)
    {
        ...
        var rows = book.Worksheet(1).RowsUsed().Skip(1);
    ...
    private static async Task ImportPickupPointsAsync(string folder)
    {
        ...
        var rows = book.Worksheet(1).RowsUsed();
        await using var conn = DatabaseService.CreateConnection();
        await conn.OpenAsync();
        await using var clear = new NpgsqlCommand("DELETE FROM pickup_points", conn);
        ...
        foreach (var row in rows)
        {
            ...
            await using var cmd = new NpgsqlCommand("INSERT INTO pickup_points (address) VALUES (@a)", conn);
    ...
    private static async Task ImportProductsAsync(string path, string imageFolder)
    {
        ...
        await using var conn = DatabaseService.CreateConnection();
        await conn.OpenAsync();
        await using var clear = new NpgsqlCommand("DELETE FROM order_items; DELETE FROM products", conn);
        await clear.ExecuteNonQueryAsync();
    ...
    private static async Task ImportOrdersAsync(string folder)
    {
        ...
        await using var conn = DatabaseService.CreateConnection();
        await conn.OpenAsync();
        await using var clear = new NpgsqlCommand("DELETE FROM order_items; DELETE FROM orders", conn);
        await clear.ExecuteNonQueryAsync();
        ...
        foreach (var row in rows)
        {
            ...
            var orderDate = ParseDate(row.Cell(3));
            var deliveryDate = ParseDate(row.Cell(4));
            ...
            const string insertOrder = """
                ...
                RETURNING id
                """;
            await using var cmd = new NpgsqlCommand(insertOrder, conn);
            ...
            await ImportOrderItemsAsync(conn, orderId, itemsRaw);
        }
    }

    private static async Task ImportOrderItemsAsync(NpgsqlConnection conn, int orderId, string itemsRaw)
    {
        ...
        for (var i = 0; i + 1 < parts.Length; i += 2)
        {
            ...
            await using var find = new NpgsqlCommand("SELECT id FROM products WHERE article=@a", conn);
            find.Parameters.AddWithValue("a", article);
            var productId = await find.ExecuteScalarAsync();
            if (productId == null) continue;

            await using var ins = new NpgsqlCommand(
                "INSERT INTO order_items (order_id, product_id, quantity) VALUES (@o, @p, @q)", conn);
            ins.Parameters.AddWithValue("o", orderId);
            ins.Parameters.AddWithValue("p", Convert.ToInt32(productId));
