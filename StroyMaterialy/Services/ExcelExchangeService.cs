...
/// <summary>
/// Импорт и экспорт всех таблиц БД в один Excel-файл (лист на таблицу).
/// </summary>
public static class ExcelExchangeService
{
    ...
    public static async Task ExportAllAsync(string filePath, IProgress<string>? progress = null)
    {
        ...
        await WriteSheetAsync(conn, book, SheetRoles, """
            SELECT id, name FROM roles ORDER BY id
            """, r => new object?[] { r.GetInt32(0), r.GetString(1) });

        progress?.Report("Экспорт пользователей...");
        await WriteSheetAsync(conn, book, SheetUsers, """
        ...
        await WriteSheetAsync(conn, book, SheetPickupPoints, """
            SELECT id, address FROM pickup_points ORDER BY id
            """, r => new object?[] { r.GetInt32(0), r.GetString(1) });

        progress?.Report("Экспорт товаров...");
        await WriteSheetAsync(conn, book, SheetProducts, """
        ...
        await WriteSheetAsync(conn, book, SheetOrders, """
        ...
        await WriteSheetAsync(conn, book, SheetOrderItems, """
    ...
    public static async Task ImportAllAsync(string filePath, IProgress<string>? progress = null)
    {
        ...
        var imageFolder = Path.GetDirectoryName(filePath) ?? AppConfig.ImportPath;
        ...
        try
        {
            ...
            await ExecuteAsync(conn, tx, "DELETE FROM order_items");
            await ExecuteAsync(conn, tx, "DELETE FROM orders");
            await ExecuteAsync(conn, tx, "DELETE FROM products");
            await ExecuteAsync(conn, tx, "DELETE FROM pickup_points");
            await ExecuteAsync(conn, tx, "DELETE FROM users");

            if (book.Worksheets.TryGetWorksheet(SheetRoles, out var rolesWs))
            {
                progress?.Report("Импорт ролей...");
                await ImportRolesAsync(conn, tx, rolesWs);
            }

            if (book.Worksheets.TryGetWorksheet(SheetPickupPoints, out var ppWs))
            {
                progress?.Report("Импорт пунктов выдачи...");
                await ImportPickupPointsAsync(conn, tx, ppWs);
            }

            if (book.Worksheets.TryGetWorksheet(SheetProducts, out var prodWs))
            {
                progress?.Report("Импорт товаров...");
                await ImportProductsAsync(conn, tx, prodWs, imageFolder);
            }

            if (book.Worksheets.TryGetWorksheet(SheetUsers, out var usersWs))
            {
                progress?.Report("Импорт пользователей...");
                await ImportUsersAsync(conn, tx, usersWs);
            }

            if (book.Worksheets.TryGetWorksheet(SheetOrders, out var ordersWs))
            {
                progress?.Report("Импорт заказов...");
                await ImportOrdersAsync(conn, tx, ordersWs);
            }

            if (book.Worksheets.TryGetWorksheet(SheetOrderItems, out var itemsWs))
            {
                progress?.Report("Импорт позиций заказов...");
                await ImportOrderItemsAsync(conn, tx, itemsWs);
            }
    ...
    private static async Task WriteSheetAsync(
        NpgsqlConnection conn,
        XLWorkbook book,
        string sheetName,
        string sql,
        Func<NpgsqlDataReader, object?[]> mapRow)
    {
        ...
        for (var c = 0; c < colCount; c++)
            ws.Cell(rowIndex, c + 1).Value = reader.GetName(c);
        ...
        while (await reader.ReadAsync())
        {
            ...
            for (var c = 0; c < values.Length; c++)
                SetCell(ws.Cell(rowIndex, c + 1), values[c]);
        }
    ...
    private static async Task ImportRolesAsync(NpgsqlConnection conn, NpgsqlTransaction tx, IXLWorksheet ws)
    {
    ...
    private static async Task ImportPickupPointsAsync(NpgsqlConnection conn, NpgsqlTransaction tx, IXLWorksheet ws)
    {
        foreach (var row in ws.RowsUsed().Skip(1))
        {
            ...
            {
                await using var cmd = new NpgsqlCommand("INSERT INTO pickup_points (address) VALUES (@a)", conn, tx);
                cmd.Parameters.AddWithValue("a", address);
                await cmd.ExecuteNonQueryAsync();
            }
        }
    ...
    private static async Task ImportProductsAsync(
        NpgsqlConnection conn, NpgsqlTransaction tx, IXLWorksheet ws, string imageFolder)
    {
        ...
        foreach (var row in ws.RowsUsed().Skip(1))
        {
            ...
            {
                var src = Path.Combine(imageFolder, imageFile);
                if (File.Exists(src))
                    File.Copy(src, Path.Combine(destFolder, imageFile), true);
            }
    ...
    private static async Task ImportUsersAsync(NpgsqlConnection conn, NpgsqlTransaction tx, IXLWorksheet ws)
    {
        foreach (var row in ws.RowsUsed().Skip(1))
        {
    ...
    private static async Task ImportOrdersAsync(NpgsqlConnection conn, NpgsqlTransaction tx, IXLWorksheet ws)
    {
        foreach (var row in ws.RowsUsed().Skip(1))
        {
    ...
    private static async Task ImportOrderItemsAsync(NpgsqlConnection conn, NpgsqlTransaction tx, IXLWorksheet ws)
    {
        foreach (var row in ws.RowsUsed().Skip(1))
        {
            var orderNumber = GetCellInt(row, 2);
    ...
    private static async Task ResetSequencesAsync(NpgsqlConnection conn, NpgsqlTransaction tx)
    {
